...
def analyse_with_pyspark(data, filename):
    ...
    m2byloc.orderBy(col('avg_price_per_m2')).coalesce(1).write.mode('overwrite').csv(f'data/{filename}_avg_price_per_m2', header=True)

    # Average price by location
    pbyloc = df.groupBy('Location').agg(round(avg('Price'), 2).alias('avg_price'))
    pbyloc.orderBy(col('avg_price')).coalesce(1).write.mode('overwrite').csv(f'data/{filename}_avg_price', header=True)

    # Offers count per location
    offcount = df.groupBy('Location').agg(count('*').alias('offer_count'))
    offcount.orderBy(col('offer_count').desc()).coalesce(1).write.mode('overwrite').csv(f'data/{filename}_offer_count', header=True)

    # Top 5 largest flats
    df.orderBy(col('m2').desc()).limit(5).coalesce(1).write.mode('overwrite').csv(f'data/{filename}_top5_largest_flats', header=True)

    # Top 5 smallest flats
    df.orderBy(col('m2').asc()).limit(5).coalesce(1).write.mode('overwrite').csv(f'data/{filename}_top5_smallest_flats', header=True)
    ...
    stats_df.coalesce(1).write.mode('overwrite').csv(f'{filename}_stats', header=True)
